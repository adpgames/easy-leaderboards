...
To get started download the asset and enable it, then go to either [adpgames.com/leaderboards](https://adpgames.com/leaderboards) or [themaker6.pythonanywhere.com/leaderboards](https://themaker6.pythonanywhere.com/leaderboards) and create your first leaderboard, or run `Leaderboard.create_leaderboard(nameOfLeaderboard)` replacing "nameOfLeaderboard" with the name as a string. There is a optional parameter for this function: "direction".  
"direction" can be "Ascending" "Descending" or "NoChange" where ascending sorts the scores lowest to highest, but no sorting will be done for string values. Instead, they will always be before the number values, and in the order they were added. Descending sorts the values highest to lowest. "NoChange" will not change the order, so it is whatever order you put the scores in.
...
Whenever you'd like to get scores from a leaderboard, make sure a leaderboard is set, and a function is connected to `Leaderboard.received_data`. Then call `Leaderboard.get_scores()`, and in the function connected to the signal "received_data", set the scores value of the BaseLeaderboard node to the list in the variable "data" that was sent in the signal.
...
If you feel like it, I would love to hear your feedback on the General tab on this repository!
